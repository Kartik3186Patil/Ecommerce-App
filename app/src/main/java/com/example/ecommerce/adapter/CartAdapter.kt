...
import com.example.ecommerce.databinding.LayoutCartItemBinding
...
class CartAdapter(val context:Context,val list:List<ProductModel>):
    ...
    override fun onBindViewHolder(holder: CartViewHolder, position: Int) {
       Glide.with(context).load(list[position].productImage).into(holder.binding.imageView4)
        ...
        holder.binding.layoutCartDelete.setOnClickListener{
            GlobalScope.launch(Dispatchers.IO) {
                dao.deleteProduct(
                    ProductModel(
                        list[position].productId,list[position].productName,list[position].productImage,list[position].productSp
                    )
                )
